...
Maintenant, assemblons le tout dans `server.js`.

[source,javascript]
----
// ... (imports et configuration en haut du server.js) ...
...
2.  Ajoutez les routes correspondantes dans `server.js`.
...
Pour un projet plus grand, garder tout dans `server.js` devient ingérable. On refactore selon l'architecture **Modèle-Vue-Contrôleur (MVC)**.
